...
SHABBAT_CLOCKS_BASE_Y = [266, 412, 556, 701]  # נקודות ה-Y של ארבעת השעונים

SHABBAT_CLOCK_LAYOUT = {
    "DIGIT_W": 10,
    "DIGIT_H": 15,
    "DIGIT_Y_OFFSET": 17,  # אופסט מהבסיס (base_y) למיקום הספרות
    "DIGITS_X": [364, 376, 388, 400], # שמאל (עשרות שעות) עד ימין (יחידות דקות)
}

# מילון טביעות האצבע של הספרות (מבוסס על מה שנתת ל-0)
# 1 = פיקסל שחור, 0 = פיקסל לבן
DIGIT_MAPS = {
    "0": [
        (0, [3,4,5,6]),       # שורה 1: פיקסלים 3,4,5,6 שחורים
        (1, [2,3,4,5,6,7]),   # שורה 2
        (2, [1,2,7,8]),       # שורה 3
        (3, [1,2,7,8]),       # שורה 4
        (4, [0,1,8,9]), (5, [0,1,8,9]), (6, [0,1,8,9]), (7, [0,1,8,9]), # שורות 5-12
        (8, [0,1,8,9]), (9, [0,1,8,9]), (10, [0,1,8,9]), (11, [0,1,8,9]),
        (12, [1,2,7,8]),      # שורה 13
        (13, [2,3,4,5,6,7]),  # שורה 14
        (14, [3,4,5,6])       # שורה 15
    ]
}